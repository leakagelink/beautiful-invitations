...
const navItems = [
  { to: "/", icon: Compass, key: "explore" as const },
  { to: "/creations", icon: Layers, key: "creations" as const },
    ...
    <nav className="fixed inset-x-0 bottom-0 z-30 border-t border-border/70 bg-background/95 backdrop-blur-xl">
      <div className="mx-auto flex max-w-lg items-end justify-between px-6 pb-3 pt-2">
        {navItems.slice(0, 2).map((item) => (
          <NavLink key={item.to} {...item} active={pathname === item.to} label={t(item.key)} />
        ))}
        <Link
          to="/search"
...
export function AppShell({ children, title, back }: { children: ReactNode; title?: string; back?: string }) {
  return (
